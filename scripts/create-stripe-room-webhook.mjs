#!/usr/bin/env node
/**
 * Crea endpoint webhook Stripe per prenotazioni sale (mp_flow=room_booking).
 * Uso: node scripts/create-stripe-room-webhook.mjs [--vercel] [webhook-url]
 *
 * Default: Supabase Edge `stripe-room-webhook`
...
function resolveWebhookUrl() {
  ...
  if (supabaseUrl) {
    return `${supabaseUrl.replace(/\/$/, "")}/functions/v1/stripe-room-webhook`;
  }
...
const body = new URLSearchParams({
  url: webhookUrl,
  description: "MusicPro School — prenotazioni sale (room_booking)",
  "metadata[mp_product]": "musicpro-school-room-booking",
});
...
console.log(
  `  Supabase Edge (stripe-room-webhook): STRIPE_WEBHOOK_SECRET=${data.secret}`,
);
