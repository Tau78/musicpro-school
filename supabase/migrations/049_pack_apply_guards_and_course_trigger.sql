-- Audit 3 follow-up: Stripe solo webhook; leftover famiglia; no doppio pack;
-- titolare non modifica prezzo/sala/listino.

CREATE UNIQUE INDEX IF NOT EXISTS lesson_credit_ledger_pack_fee_uidx
  ON public.lesson_credit_ledger (lesson_fee_id)
  WHERE kind = 'pack' AND lesson_fee_id IS NOT NULL;

CREATE OR REPLACE FUNCTION public.guard_course_titular_columns()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  IF auth.role() = 'service_role' OR public.is_admin_or_segreteria() THEN
    RETURN NEW;
  END IF;

  IF NEW.price_eur IS DISTINCT FROM OLD.price_eur
     OR NEW.pay_amount_eur IS DISTINCT FROM OLD.pay_amount_eur
     OR NEW.pay_rate_type_id IS DISTINCT FROM OLD.pay_rate_type_id
     OR NEW.room_id IS DISTINCT FROM OLD.room_id
     OR NEW.weekly_dow IS DISTINCT FROM OLD.weekly_dow
     OR NEW.weekly_start_minute IS DISTINCT FROM OLD.weekly_start_minute
     OR NEW.duration_minutes IS DISTINCT FROM OLD.duration_minutes
     OR NEW.titular_member_id IS DISTINCT FROM OLD.titular_member_id
     OR NEW.term_id IS DISTINCT FROM OLD.term_id
     OR NEW.subject_id IS DISTINCT FROM OLD.subject_id
     OR NEW.course_kind IS DISTINCT FROM OLD.course_kind
     OR NEW.starts_on IS DISTINCT FROM OLD.starts_on
     OR NEW.max_students IS DISTINCT FROM OLD.max_students
     OR NEW.counts_as_hour IS DISTINCT FROM OLD.counts_as_hour
     OR NEW.is_trial IS DISTINCT FROM OLD.is_trial
     OR NEW.name IS DISTINCT FROM OLD.name
  THEN
    RAISE EXCEPTION 'Puoi cambiare solo lo stato del corso, non listino o orario.';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_guard_course_titular_columns ON public.courses;
CREATE TRIGGER trg_guard_course_titular_columns
  BEFORE UPDATE ON public.courses
  FOR EACH ROW
  EXECUTE FUNCTION public.guard_course_titular_columns();
