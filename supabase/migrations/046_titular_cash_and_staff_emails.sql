-- Audit V1: titolare registra contanti; email coda staff via DEFINER.
...
CREATE POLICY "lesson_pack_payments_select_titular"
  ON public.lesson_pack_payments FOR SELECT
  TO authenticated
  USING (
    public.has_member_role('docente'::public.member_role)
    AND created_by = public.current_member_id()
...
CREATE POLICY "fiscal_receipts_titular"
...
CREATE POLICY "fiscal_receipt_lines_titular"
  ON public.fiscal_receipt_lines FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.fiscal_receipts r
      WHERE r.id = receipt_id
        AND r.created_by = public.current_member_id()
    )
  )
  WITH CHECK (
    EXISTS (
...
CREATE OR REPLACE FUNCTION public.list_lesson_staff_emails()
...
AS $$
  SELECT DISTINCT ON (lower(trim(m.email)))
    trim(m.email) AS email,
    trim(concat_ws(' ', m.last_name, m.first_name)) AS label
  FROM public.members m
  JOIN public.member_roles r
    ON r.member_id = m.id
   AND r.revoked_at IS NULL
   AND r.role IN ('admin', 'segreteria')
  WHERE m.email IS NOT NULL
    AND trim(m.email) <> ''
  ORDER BY lower(trim(m.email)), m.last_name, m.first_name;
$$;

REVOKE ALL ON FUNCTION public.list_lesson_staff_emails() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.list_lesson_staff_emails() TO authenticated;
