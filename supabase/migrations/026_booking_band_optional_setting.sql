-- MusicPro School — Band obbligatoria configurabile (default: facoltativa)
-- Periodo di transizione: prenotazione singola con quota ok; switch admin per obbligatorietà.
...
COMMENT ON FUNCTION public.get_booking_setting_bool IS
  'Legge un flag booleano da app_settings (true/false, 1/0, yes/no).';

-- create_booking_safe — band obbligatoria solo se booking_band_required = true
...
COMMENT ON FUNCTION public.create_booking_safe(UUID, UUID, TIMESTAMPTZ, TIMESTAMPTZ, boolean, UUID) IS
  'Creates booking: PROVI DA SOLO, optional/required band (booking_band_required), or individual if quota ok.';
