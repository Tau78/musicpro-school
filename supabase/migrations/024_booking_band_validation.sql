-- MusicPro School — Fase 3 Wave 3.3: booking band validation
-- Depends on: 022_bands_and_quota_payments.sql

-- ---------------------------------------------------------------------------
-- bookings — band reference + member snapshot at booking time
-- ---------------------------------------------------------------------------
ALTER TABLE public.bookings
  ADD COLUMN IF NOT EXISTS band_id UUID REFERENCES public.bands(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS member_snapshot JSONB;

COMMENT ON COLUMN public.bookings.band_id IS
  'Band associata alla prenotazione (null per PROVI DA SOLO).';
COMMENT ON COLUMN public.bookings.member_snapshot IS
  'Snapshot JSON dei membri active al momento della prenotazione (ids e nomi).';

CREATE INDEX IF NOT EXISTS idx_bookings_band_id
  ON public.bookings (band_id)
  WHERE band_id IS NOT NULL;

-- ---------------------------------------------------------------------------
-- create_booking_safe — band validation + p_band_id
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.create_booking_safe(UUID, UUID, TIMESTAMPTZ, TIMESTAMPTZ, boolean, UUID) IS
  'Creates booking with overlap check, pricing, lead-time, PROVI DA SOLO or band validation.';
