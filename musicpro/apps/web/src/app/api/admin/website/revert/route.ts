...
import { getWebsiteAdminState, revertWebsiteDraft } from "@musicpro/database";
...
export async function POST() {
  ...
  const result = await revertWebsiteDraft(access.supabase);
  if (!result.success) {
    return NextResponse.json(
      { ok: false, message: result.errorMessage || "Ripristino non riuscito." },
      { status: 500 },
    );
  }
